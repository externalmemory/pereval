...
The change is still not comparable, but the quantitative case first made here does not survive. It compared the naive OLS baseline, byte-identical across both generators, normalized by its own degenerate anchor, and read 0.39 against 0.67 as evidence the task had got harder. That ratio has a sampling spread of 0.46 to 0.95 across five independent ten-instance seed sets on one unchanged configuration, so at this sample size it cannot resolve the differences it was being used to attribute. Repeated attempts to decompose the change with it produced unstable and once reversed answers. It is a noisy statistic being asked to do work it cannot do, which is the same error the Sample Size section warns about, committed in the analysis rather than in a table.

Comparability fails for structural reasons instead, none of which is a statistical estimate. One instance in three now draws a benign scenario with a stress multiple near 1.1x, a regime the old generator never produced because its severity was always drawn from [1.0, 1.8]. Two instances in three now carry a nonlinear response law, where the old task was always probit-linear. And the absolute scale moved, with the oracle going from 0.064 to 0.094. Any one of those breaks a max-over-instances cell on its own. The oracle moved from 0.064 to 0.094 under those same pinned conditions, chiefly because rho is now drawn from [0.010, 0.050] against a fixed 0.02, which rescales every Winkler number in the column. And two of three instances now carry a nonlinear law, where a correctly specified linear fit scores 0.097 and 0.185 against 0.038, which is a difficulty change an agent experiences directly.
