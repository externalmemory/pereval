...
Two attributions made earlier here were wrong and are worth recording, since both were guesses that a measurement overturned. The rise in peak stressed default rates is not caused by the scenario re-anchoring: on the comparable linear family it is 9.4 percent before and 10.0 percent after. It is caused by the nonlinear families, mechanically, since a kink adds slope above its threshold and a cross term is positive when both drivers move adversely, so both amplify the far end of the path (threshold 16.6 percent, interaction 18.4 percent). And the re-anchoring did not change the level of scenario severity at all; it changed its dispersion, which was the point. Terminal severity of the unemployment driver went from 2.20 sd with a spread of 1.71 to 1.93 sd with a spread of 0.96, and the old scheme produced instances whose stress scenario ended BELOW the pre-stress mean, at -0.47 sd, which tested no extrapolation at all.
