...
Pricing is not enough on its own, because a penalty is still a number and a number still gets averaged into a cell. Some runs are not measurements at all and must not be reported as scores.

The line is whether the failure was the agent's. A run is **unmeasured** when it failed for reasons outside the agent's control: no assistant turn at all, a provider error, or termination at the message or time cap. A run where the agent worked inside its budget and still produced nothing usable **is** a measurement, and the degenerate penalty is the right price for it.

The distinction is not academic, and both halves have been observed on the same model. In July deepseek-v4-flash-free returned two messages with no assistant turn on nine orbital runs, which is infrastructure and says nothing about the model. Re-run in the same configuration months later it worked through 36 to 194 messages, well inside its cap, and on some instances still never wrote a parseable predictions.csv. That second thing is capability, and hiding it behind `n/m` would flatter the model exactly as much as scoring the first would malign it.
