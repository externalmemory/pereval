...
| Kimi K3 (paid) | 0.12 | 14 | 0.094 | **3.3** | **8.2** (n=1) | 0.083 (n=1)⁴ | *not ranked* |
...
⁴ Kimi K3 is the only **frontier** model in the table, and it is not the only paid one: Claude Haiku 4.5 is also paid. It separates from the rest of the cast by an order of magnitude on both hard orbital tasks, which is the only kind of gap this suite can resolve. Three-body: **3.3** against an oracle of 0.039, where the next model is GLM-5.1 at 436. Flyby: **8.2** against a next-best of 292 (mimo) and a degenerate answer of 138, making it the only model in the suite that beats a constant there.

The flyby cell is n=1 because two of the three runs hit the wall clock, but the conclusion does not rest on which run is quoted: those two scored 5.5 and 150.7, so all three sit at or below the best cell any other model achieved. Quantile is also n=1, at 0.083, which is indistinguishable from the rest of that column. Two-body (0.094) is level with GLM's 0.092. The row stops at five cells because the budget did; the cost post-mortem is in [docs/limitations.md](docs/limitations.md#cost-estimates-do-not-survive-a-change-of-limits).
...
- **Degenerate answer**: one constant point estimate, no interval. It reads differently by column, and that is the point of having it. Every one of the eight ranked rows is worse than a constant on Flyby, and ling is worse than one on three-body too (3309 against 3019) despite posting the lowest quantile score. The single exception anywhere in the Flyby column is Kimi K3, at 8.2 against that instance's degenerate answer of 107.8. In the Quantile column it is the same estimator as the naive row, `np.percentile`.
