...
### Example Scores (CCAR Task; Eight Instances)

Unlike the single-instance tables below, these are means over eight generated instances with standard errors, so the ordering is more robust (still a small sample). Every row runs the same eight instances (seed 1), so the comparison is paired. Lower Winkler regret is better; coverage targets 0.95. The Vasicek reference and naive OLS baseline (`-T baseline=vasicek|naive`) bracket the task.

| Row | Winkler regret ± SE | Coverage | Note |
| --- | --- | --- | --- |
| Vasicek reference (true model) | 0.013 ± 0.004 | 0.93 | closed-form extended Vasicek |
| Kimi K3 | 0.033 ± 0.014 | 0.90 | frontier (not free); best model, ties hy3-free within SE |
...
Every model that completed beats the naive OLS baseline, and the best of them approach the near-oracle Vasicek reference, so CCAR is tractable even for cheap models: a straightforward regression under noisy data. The task still discriminates the right way, with the fragile linear-on-levels approach worst and the physics-informed reference best. It is the suite's easy realistic task, in deliberate contrast to three-body. Kimi K3, the one frontier model run here, lands at 0.033 ± 0.014, best of the non-reference rows but statistically indistinguishable from hy3-free; its coverage (0.90) dips slightly below target, dragged by one of the eight scenarios where it was overconfident (regret 0.12, coverage 0.68). (nemotron-3-ultra-free and north-mini-code-free errored on this run and are omitted.)
