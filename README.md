...
⁵ `deepseek-v4-flash-0731` is a **new row, not a repair of the old one**. It is a different model version from `deepseek-v4-flash-free`, whose row had three unmeasured cells and has been removed rather than carried alongside; the removed row's failures are still documented in [limitations.md](docs/limitations.md#non-response-was-cheaper-than-failure), which is where the evidence for the non-response penalty floor lives. The three cells the old row did have moved in every direction (CCAR 0.14 to 0.30, Ballistic 75 to 14, Quantile 0.17 to 0.067), but none of those is a clean version comparison: they were measured at lower message caps, and the CCAR figure partly under the previous answer key.

What the row establishes on its own evidence is that this model clears the constant almost everywhere. It beats the per-instance degenerate answer on **every run of every task except two of three on the Flyby**, including all three three-body runs, which most of the cast fails. Its Flyby cell of 522 is nonetheless worse than a constant, because the cell is the worst case and the one good run (31.9 against an anchor of 127.0) does not set it. One three-body instance hit the 500-message cap and was re-run under a 1200 ceiling, where it finished in **190 messages** and scored 269; the capped attempt's 340.3 is discarded, since a truncated run is not a measurement. That instance is the most message-hungry single run in the suite's history, against a previous maximum of 182 on this task. The full six-cell row cost **$1.28**.
...
- **`n/m`**: at least one run failed for reasons outside the agent's control, so the cell is unmeasured rather than scored. An agent that works inside its budget and still produces nothing usable is scored, at the degenerate answer. No cell in the table is currently `n/m`. The convention earned its place on deepseek-v4-flash-free, whose previously best-in-suite three-body and flyby figures turned out to be penalties from an upstream failure rather than results; that row has since been superseded by the 0731 version above, and the episode is kept in [limitations.md](docs/limitations.md#non-response-was-cheaper-than-failure) because it is the evidence for the penalty floor ([detail](docs/limitations.md#non-response-was-cheaper-than-failure)).
- **Degenerate answer**: one constant point estimate, no interval. It reads differently by column, and that is the point of having it. Nine of the ten ranked rows are worse than a constant on Flyby, and ling is worse than one on three-body too (3309 against 3019) despite posting the lowest quantile score. The only row whose Flyby cell clears its anchor is Kimi K3, whose three runs beat theirs by 2.8x, 10.8x and 6.1x. Per run rather than per cell there is one other: deepseek-v4-flash-0731 cleared its anchor on one of three Flyby runs by 4.0x, and was three to four times worse than a constant on the other two, which is what a worst-case cell is for. In the Quantile column it is the same estimator as the naive row, `np.percentile`.
- **Mean rank**: Kimi K3 leads at 2.00, deepseek-v4-flash-0731 is second at 3.17, and the naive baseline is eighth of ten. Permuting ranks within each column, the null of no cross-task skill, reproduces the observed spread at p = 0.004 and the observed range at p = 0.007, down from 0.037 and 0.013 before the deepseek row was added and from p = 0.57 before K3's row was complete. Dropping rows one at a time shows the two are not equal partners in that: without K3 the remaining nine give p = 0.090, without deepseek the remaining nine still give p = 0.037, and without both the other eight collapse to p = 0.561. So K3 alone still carries a detectable signal, deepseek alone does not quite, and everything below them remains as unordered as it ever was. The caveat is therefore unchanged in kind: the aggregate detects that the top one or two models are genuinely different, not that the rest are ranked ([why the aggregate is weak](docs/limitations.md#the-overall-rank-depends-on-the-task-mix)).
